...
class KungFuElasticTrainHook(tf.train.SessionRunHook):
    ...
    def begin(self):
        self._kungfu_step = tf.Variable(0, trainable=False, name="kungfu_step", dtype=tf.int64)
        self._changed = tf.Variable(0, trainable=False, name="changed", dtype=tf.int64)
        self._advance = tf.assign_add(self._kungfu_step, 1)
        self._has_changed = tf.assign(self._changed, 1)
        self._has_not_changed = tf.assign(self._changed, 0)
    ...
    def after_run(self, run_context, run_values):
        ...
        if changed:
            run_context.session.run(self._has_changed)
            print('changed on %d' % (kungfu_step))
            self._need_sync = True
            if not keep:
                run_context.request_stop()
                return
        run_context.session.run(self._has_not_changed)
